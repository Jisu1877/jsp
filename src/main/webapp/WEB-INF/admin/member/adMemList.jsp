...
<div class="container">
	...
	<table class="table table-hover text-center">
		...
		<c:forEach var="vo" items="${vos}" varStatus="st">
			<tr>
				<td>${curScrStartNo}</td>
				<td><a href="${ctp}/memInfor.mem?mid=${vo.mid}&applyDiff=${vo.applyDiff}">${vo.mid}</a></td>
				<td>${vo.nickName}</td>
				<td>${vo.name}</td>
				<td>${vo.userInfor}</td>
				<td>
				<%-- 
					<c:if test="${vo.level == 0}">
						<c:set var="strLevel" value="관리자"/>
					</c:if>
					<c:if test="${vo.level == 1}">
						<c:set var="strLevel" value="준회원"/>
					</c:if>
					<c:if test="${vo.level == 2}">
						<c:set var="strLevel" value="정회원"/>
					</c:if>
					<c:if test="${vo.level == 3}">
						<c:set var="strLevel" value="우수회원"/>
					</c:if>
					<c:if test="${vo.level == 4}">
						<c:set var="strLevel" value="운영자"/>
					</c:if>
					${strLevel}
					 --%>
