...
insert into board values (default, '관리투', '게시판 서비스를 시작합니다.', 'ljs1877@gmail.com', 'http://blog.daum.net/cjsk1126', '이곳은 게시판입니다.', default, default, '192.168.50.103', default, 'admin');

select * from board;

select * from board where idx = 5 - 1; /*현재글이 5번글일 경우 */
select * from board where idx = 20 + 1;
...
/* 최근글 출력연습 */
select * from board where date_sub(now(), interval 1 month) < wDate order by idx desc;
select count(*) from board where date_sub(now(), interval 1 day) < wDate;
select * from board where 1 > 2; /*where절 이하가 false가 되어서 데이터가 select되지 않는다.*/
...
create table boardReply (
	...
	foreign key(boardIdx) references board(idx) /*board테블의 idx를 boardReply테이블의 외래키(boardIdx)로 설정 */
...
-- boardReply 테이블의 구조?
desc boardReply;
-- board 테이블의 idx 29번글에 적혀있는 댓글(boardReply테이블)의 개수는?
select count(*) from boardReply where boardIdx = 29;
-- board 테이블의 idx 29번글에 적혀있는 댓글(boardReply테이블)의 개수는?(단, 출력은 부모글의 고유번호와 댓글개수를 출력)
select boardIdx as '부모고유번호', count(*) as '댓글개수' from boardReply where boardIdx = 29;
-- board 테이블의 idx 29번글에 적혀있는 댓글(boardReply테이블)의 개수는?(단, 출력은 부모글의 고유번호와 댓글개수, 게시글을 쓴 사람의 nickName 까지)
select (select nickName from board where idx = 29) as 닉네임, boardIdx as 부모고유번호, count(*) as 댓글개수 from boardReply where boardIdx = 29;
-- board 테이블의 29번의 작성자의 아이디와 현재글(부모글 29번)에 달려있는 댓글 사용자의 닉네임을 출력하시오.
select (select mid from board where idx = 29) as 작성자아이디, nickName from boardReply where boardIdx = 29;

-- 부모관점에서 자료를 select 해오면.. 1:多 관계라서, 댓글이 2개 이상이면..에러가 난다.(아이디는 1개가 출력.댓글은 2개 출력이기 때문에..) 
select mid, (select nickName from boardReply where boardIdx = 29) as 작성자닉네임 from board where idx = 29;

-- board 테이블의 29번의 작성자의 아이디와 현재글(부모글 29번)에 달려있는 댓글의 갯수를 출력하시오.
select mid, (select count(*) from boardReply where boardIdx = 29) as 댓글개수 from board where idx = 29;

-- 부모테이블(board)에 있는 자료 중 뒤에서 5개를 출력하되(아이디,닉네임), 부모테이블에 달려있는 댓글(boardReply)의 수와 함께 출력하시오.
