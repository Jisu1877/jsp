...
<div class="container">
	...
	<div class="text-center">
		<c:if test="${flag == 's'}">
			<input type="button" value="돌아가기" onclick="location.href='boSearch.bo?pag=${pag}&pageSize=${pageSize}&search=${search}&searchString=${searchString}';" class="btn w3-amber"/> <!-- param을 안쓰고 커맨드객체에서 request로 값 보내는 방법이 있다. -->
		</c:if>
		<c:if test="${flag != 's'}">
			<input type="button" value="돌아가기" onclick="location.href='boList.bo?pag=${pag}&pageSize=${pageSize}';" class="btn w3-amber"/> <!-- param을 안쓰고 커맨드객체에서 request로 값 보내는 방법이 있다. -->
		</c:if>
		<c:if test="${sMid == vo.mid || sLevel == 0}">
			<input type="button" value="수정하기" onclick="location.href='boUpdate.bo?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn w3-amber"/>
